...
public class ProfitReportController {
	...
	public String getProfitReport(Model model, HttpServletRequest rq,
			HttpServletResponse response, @RequestParam("code") int code,
			@RequestParam("state") boolean state) throws SQLException {
		Map<?, ?> map = reportAdapterService.getParameters(rq, state, "Profit");
		...
		if (!state) {
			return "/WEB-INF/reports/Profit.jasper";
		} else {
			return "/WEB-INF/reports/ProfitCompleted.jasper";
		}
	}
}
